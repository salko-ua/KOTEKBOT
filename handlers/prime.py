...
async def cancel(query: types.CallbackQuery, state: FSMContext):
    await query.message.edit_text(
        "Редагування тексту успішно відмінено✅", reply_markup=text_inline_kb
    )
    await state.finish()
    ...
    if await db.user_exists_sql(message.from_user.id):
        if len(message.text) <= 1:
            await message.answer(
                "Текст надто короткий спробуй ще раз", reply_markup=text_inline_kb
            )
            await state.finish()
        else:
            text = message.text
            ...
            await message.answer(
                "Замітку успішно додано ✅", reply_markup=text_inline_kb
            )
            await state.finish()
...
async def write(message: types.Message):
    ...
    msg = await message.answer(
        "Видалення клавіатури", reply_markup=types.ReplyKeyboardRemove()
    )
    await msg.delete()
    await message.answer(
...
async def write_group_message_text(message: types.Message, state: FSMContext):
    group, group_user_writer, all_user_us, all_user_their = await get_data(
        message, state
    )
    text = message.text
    data = None
...
async def register_handler_stats(dp: Dispatcher):
    ...
    dp.register_message_handler(
        write, Text(ignore_case=True, equals="Написати ✉️"), state=None
    )

    # Cкасувати вибір групи
    dp.register_callback_query_handler(
        back_write_group, text="Назад", state=FSMWrite.group
    )
    # Вибрав групу
    for text in await get_list():
        dp.register_callback_query_handler(
            write_group, lambda c, t=text: c.data == t, state=FSMWrite.group
        )

    # Повернутись до вибору групи
    dp.register_callback_query_handler(
        back_write_group_message, text="інша", state=FSMWrite.message_group
    )
    # Надіслав повідомлення
    dp.register_message_handler(
        write_group_message_text, content_types=["text"], state=FSMWrite.message_group
    )
    dp.register_message_handler(
        write_group_message_photo, content_types=["photo"], state=FSMWrite.message_group
    )
    dp.register_message_handler(
        write_group_message_sticker,
        content_types=["sticker"],
        state=FSMWrite.message_group,
    )
    dp.register_message_handler(
        write_group_message_video, content_types=["video"], state=FSMWrite.message_group
    )
    dp.register_message_handler(
        write_group_message_animation,
