...
async def settings(message: types.Message) -> None:
    ...
    if not await db.student_exists_sql(user_id):
        await message.answer(
            "Ви не зареєстровані! ❌", reply_markup=await Keyboards.hide_kb()
        )
        return

    await message.answer(
        "Налаштуйте свій акаунт в боті:",
        reply_markup=await Keyboards.settings_inile_kb(user_id),
    )
...
async def change_student_group(query: types.CallbackQuery, state: FSMContext) -> None:
    ...
    await query.message.edit_reply_markup(
        reply_markup=await Keyboards.student_group_list_kb()
    )
