...
async def is_super_admin(message: types.Message) -> None:
...
async def super_admin(message: types.Message) -> None:
    ...
    await message.answer("Клавіатура власника", reply_murkup=super_admin_kb())
...
async def add_calls1(message: types.Message, state: FSMContext) -> None:
    ...
    await message.answer("Розклад дзвінків оновлено ✅", reply_murkup=super_admin_kb())
    await state.clear()
...
async def delete_calls(message: types.Message) -> None:
    ...
    if not check:
        await message.answer("Розкладу дзвінків ще немає 🔴", reply_murkup=super_admin_kb())
        return

    await message.answer("Розклад дзвінків видалено 🗑", reply_murkup=super_admin_kb())
...
async def delete_group(message: types.Message, state: FSMContext) -> None:
    ...
    await message.answer("Виберіть групу ⬇️", reply_murkup=await group_selection_student_kb())
...
async def delete_group1(message: types.Message, state: FSMContext) -> None:
    ...
    if message.text == "Назад":
        await message.answer("Меню", reply_murkup=super_admin_kb())
        await state.clear()
        return

    fullname = message.text
    if not await db.student_group_exists(fullname):
        await message.answer("Група з такою назвою немає", reply_murkup=super_admin_kb())
        await state.clear()
        return

    if len(fullname) >= 3:
        await message.answer("❌ Ліміт 3 символи ", reply_murkup=super_admin_kb())
        await state.clear()
        return

    if not await db.student_in_group_exists(fullname):
        await db.delete_student_group(fullname)
        await message.answer("Групу видалено ✅", reply_murkup=super_admin_kb())
        await state.clear()
        return
...
async def add_group1(message: types.Message, state: FSMContext) -> None:
    ...
    if await db.student_group_exists(name):
        await message.answer("Група з такою назвою вже є", reply_murkup=super_admin_kb())
        await state.clear()
        return
    ...
    await message.answer("Групу створено ✅", reply_murkup=super_admin_kb())
    await state.clear()
...
async def add_schedule_to_group1(message: types.Message, state: FSMContext) -> None:
    ...
    await message.answer("До якої групи привязати", reply_murkup=await group_selection_student_kb())
...
async def add_schedule_to_group2(message: types.Message, state: FSMContext) -> None:
    ...
    if message.text == "Назад":
        await message.answer("Клавіатура", reply_murkup=super_admin_kb())
        await state.clear()
        return

    if not await db.student_group_exists(message.text):
        await message.answer("Немає такої групи ❌", reply_murkup=super_admin_kb())
        await state.clear()
        return
    ...
    await message.answer("Розклад успішно додано ✅", reply_murkup=super_admin_kb())
    await state.clear()
...
async def delete_photo_group1(message: types.Message, state: FSMContext) -> None:
    ...
    if not await db.student_group_exists(message.text):
        await message.answer("Групи з такою назвою немає", reply_murkup=super_admin_kb())
        await state.clear()
        return

    await db.delete_student_group_photo(message.text)
    await message.answer("Розклад групі успішно видалено", reply_murkup=super_admin_kb())
