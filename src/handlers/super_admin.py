...
async def super_admin_delete_user(message: types.Message, state: FSMContext) -> None:
    ...
    await message.answer(text="Введіть ID студента")
...
async def super_admin_delete_user1(message: types.Message, state: FSMContext) -> None:
    ...
    if exits:
        ...
        await message.answer(text="Студента видаленно")
        await state.clear()
    elif not exits:
        await message.answer(text="Немає користувача з таким ID")
        await state.clear()
...
async def add_calls(message: types.Message, state: FSMContext) -> None:
    ...
    await message.answer(text="Завантажте фото", reply_markup=ReplyKeyboardRemove())
...
async def add_calls1(message: types.Message, state: FSMContext) -> None:
    ...
    await message.answer(text="Розклад дзвінків оновлено ✅", reply_markup=super_admin_kb())
    await state.clear()
...
async def delete_calls(message: types.Message) -> None:
    ...
    if not check:
        await message.answer(text="Розкладу дзвінків ще немає 🔴", reply_markup=super_admin_kb())
        return

    await message.answer(text="Розклад дзвінків видалено 🗑", reply_markup=super_admin_kb())
...
async def delete_group(message: types.Message, state: FSMContext) -> None:
    ...
    await message.answer(text="Виберіть групу ⬇️", reply_markup=await group_selection_student_kb())
...
async def delete_group1(message: types.Message, state: FSMContext) -> None:
    ...
    if message.text == "Назад":
        await message.answer(text="Меню", reply_markup=super_admin_kb())
        await state.clear()
        return

    fullname = message.text
    if not await db.student_group_exists(fullname):
        await message.answer(text="Група з такою назвою немає", reply_markup=super_admin_kb())
        await state.clear()
        return

    if len(fullname) >= 3:
        await message.answer(text="❌ Ліміт 3 символи ", reply_markup=super_admin_kb())
        await state.clear()
        return

    if not await db.student_in_group_exists(fullname):
        await db.delete_student_group(fullname)
        await message.answer(text="Групу видалено ✅", reply_markup=super_admin_kb())
        await state.clear()
        return
...
async def add_group1(message: types.Message, state: FSMContext) -> None:
    ...
    if await db.student_group_exists(name):
        await message.answer(text="Група з такою назвою вже є", reply_markup=super_admin_kb())
        await state.clear()
        return
    ...
    await message.answer(text="Групу створено ✅", reply_markup=super_admin_kb())
    await state.clear()
...
async def add_schedule_to_group(message: types.Message, state: FSMContext) -> None:
    ...
    await message.answer(text="Киньте фото розкладу", reply_markup=ReplyKeyboardRemove())
...
async def add_schedule_to_group1(message: types.Message, state: FSMContext) -> None:
    ...
    await message.answer(
        text="До якої групи привязати", reply_markup=await group_selection_student_kb()
    )
...
async def add_schedule_to_group2(message: types.Message, state: FSMContext) -> None:
    ...
    if message.text == "Назад":
        await message.answer(text="Клавіатура", reply_markup=super_admin_kb())
        await state.clear()
        return

    if not await db.student_group_exists(message.text):
        await message.answer(text="Немає такої групи ❌", reply_markup=super_admin_kb())
        await state.clear()
        return
    ...
    await message.answer(text="Розклад успішно додано ✅", reply_markup=super_admin_kb())
    await state.clear()
...
async def delete_photo_group1(message: types.Message, state: FSMContext) -> None:
    ...
    if not await db.student_group_exists(message.text):
        await message.answer(text="Групи з такою назвою немає", reply_markup=super_admin_kb())
        await state.clear()
        return

    await db.delete_student_group_photo(message.text)
    await message.answer(text="Розклад групі успішно видалено", reply_markup=super_admin_kb())
