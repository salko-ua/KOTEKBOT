...
async def delete_student_group_callback(query: types.CallbackQuery, state: FSMContext):
    ...
    if query.data == "admin_back_kb":
        await query.message.edit_text(f"Ви повернулись назад", reply_markup=super_admin_back_kb())
        await state.clear()
        return
...
#         "Виберіть групу зі списку ⬇️", reply_markup=await group_selection_student_kb()
#     )
#     await state.set_state(FSMSuperAdminPanel.delete_schedule_photo_group)
    

# @router.callback_query(F.data, FSMSuperAdminPanel.delete_schedule_photo_group)
# async def add_or_change_schedule2(message: types.Message, state: FSMContext):
#     db = await Database.setup()
#     data = (await state.get_data())["name_group"]
    

#     await message.answer("Фото групи видалено ✅", reply_markup=super_admin_schedule())
#     await clear_all(message, state)

#     await db.delete_student_group_photo(data)
    
    
    
# Обробник для кнопки "Видалити 👥"
@router.callback_query(F.data == "Видалити 🗓")
async def delete_student_schedule(query: types.CallbackQuery, state: FSMContext):
    await query.message.edit_text(
        "Виберіть групу зі списку нижче ⬇️",
        reply_markup=await group_selection_student_kb(),
    )

    await state.set_state(FSMSuperAdminPanel.delete_group_schedule)


@router.callback_query(F.data, FSMSuperAdminPanel.delete_group_schedule)
async def delete_student_schedule_callback(query: types.CallbackQuery, state: FSMContext):
    db = await Database.setup()
    group_name = query.data
    print(query.data)
    if query.data == "admin_back_kb":
    ...
    await db.delete_student_group_photo(group_name)

    await query.message.edit_text(
        f"Розклад {group_name} видалений ✅", reply_markup=super_admin_back_kb()
    )
    await state.clear()
